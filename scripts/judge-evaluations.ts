  ...
  try {
    ...
    if (seed) {
      ...
      const batchProvider = batch
        ? await createBatchProvider({ model: HAIKU_MODEL, maxTokens: JUDGE_MAX_TOKENS, temperature: JUDGE_DEFAULT_TEMPERATURE })
        : undefined;
      const llm = batchProvider ?? await createAnthropicProvider();
