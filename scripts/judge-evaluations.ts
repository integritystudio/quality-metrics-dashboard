...
export type JudgeMessagesClient = {
  messages: {
    create(params: AnthropicSdk.MessageCreateParamsNonStreaming): Promise<Pick<AnthropicSdk.Message, 'content'>>;
  };
};
...
async function createAnthropicProvider(): Promise<LLMProvider> {
  ...
  return anthropicProviderFor(new Anthropic());
}

/** The judge's provider over an SDK client, or a fake one in tests. */
export function anthropicProviderFor(client: JudgeMessagesClient): LLMProvider {
  return {
    async generate(
      prompt: string,
